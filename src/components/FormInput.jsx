...
export default function FormInput({ label, hint, className = '', ...props }) {
  return (
    <label className="flex min-w-0 flex-col gap-1.5 text-sm text-marron-cafe">
      ...
      <input
        {...props}
        className={`min-w-0 rounded-xl border border-marron-tierra/20 bg-white px-3 py-2 text-sm text-marron-cafe outline-none transition-colors duration-150 focus-visible:border-verde-lima disabled:bg-marron-tierra/5 disabled:text-marron-cafe/50 ${className}`}
      />
