...
// Punto 1 del relevamiento (sección 2.6): humedad tomada cada 30 min a la
// salida del secador 2, con alerta fuera de 7%-15%. El backend todavía no
// tiene un campo para esto (solo persiste avgDryer1TempC/avgDryer2TempC al
// cierre) — mientras tanto es un registro 100% local, no se envía al crear
// ni al cerrar la entrada.
const HUMEDAD_MIN_ALERTA = 7
const HUMEDAD_MAX_ALERTA = 15
...
export default function ControlVolumenA({ loteInicialId }) {
  ...
  // Registro de humedad cada 30 min — mock local, ver nota de HUMEDAD_MIN_ALERTA.
  // entryId -> [{ hora, valor }]
  const [lecturasHumedad, setLecturasHumedad] = useState({})
  const [humedadInput, setHumedadInput] = useState({})
  ...
  const registrarLecturaHumedad = (entryId) => {
    const valor = humedadInput[entryId]
    if (valor == null || valor === '') return
    const lectura = { hora: new Date().toLocaleTimeString('es-BO', { hour: '2-digit', minute: '2-digit' }), valor: Number(valor) }
    setLecturasHumedad((prev) => ({ ...prev, [entryId]: [...(prev[entryId] ?? []), lectura] }))
    setHumedadInput((prev) => ({ ...prev, [entryId]: '' }))
  }
        ...
        {!form.lotId ? (
          ...
          <div className="flex flex-col gap-3">
            {entradasAbiertas.map((e) => {
              ...
              const lecturas = lecturasHumedad[e.id] ?? []
              const promedioHumedad = lecturas.length
                ? lecturas.reduce((acc, l) => acc + l.valor, 0) / lecturas.length
                : null
              return (
                <div key={e.id} className="flex flex-col gap-3 rounded-2xl bg-white/70 p-4">
                  ...
                  <div className="flex flex-col gap-2 border-t border-marron-tierra/15 pt-3">
                    <MockupBanner mensaje={`Mockup — humedad a la salida del secador 2, cada 30 min. Alerta fuera de ${HUMEDAD_MIN_ALERTA}%-${HUMEDAD_MAX_ALERTA}%. No se envía al backend todavía.`} />
                    <div className="flex flex-wrap items-end gap-3">
                      <FormInput
                        label="Humedad (%)"
                        type="number"
                        ...
                        min="0"
                        max="100"
                        value={humedadInput[e.id] ?? ''}
                        onChange={(ev) => setHumedadInput((prev) => ({ ...prev, [e.id]: ev.target.value }))}
                        className="w-32"
                      />
                      <Button variant="secondary" className="px-4 py-2 text-xs" onClick={() => registrarLecturaHumedad(e.id)}>
                        Agregar lectura
                      </Button>
                      {promedioHumedad != null && (
                        <span className="text-xs font-semibold text-marron-cafe">
                          Promedio: {promedioHumedad.toFixed(1)}%
                        </span>
                      )}
                    </div>
                    {lecturas.length > 0 && (
                      <ul className="flex flex-wrap gap-2">
                        {lecturas.map((l, i) => {
                          const fueraDeRango = l.valor < HUMEDAD_MIN_ALERTA || l.valor > HUMEDAD_MAX_ALERTA
                          return (
                            <li key={i}>
                              <Badge tono={fueraDeRango ? 'negativo' : 'positivo'}>
                                {l.hora} · {l.valor}%
                              </Badge>
                            </li>
                          )
