...
// Modal "Registrar salida" — es el formulario completo P-ADM-03/R-24 (Nota
// de Entrega de Materia Prima), abierto desde la lista de entregas (ver
// SeccionEntregaMateriaPrima.jsx). Antes vivía como pantalla propia de la
// subpestaña "Entrega"; ahora esa subpestaña muestra el listado de entregas
// ya registradas y este modal es el único punto para cargar una nueva. Real:
// cada nota es un POST /warehouse-deliveries (documentType='R-24'), ver
...
// (GET /iam/users?roleCode=produccion).
export default function ModalEntregaMateriaPrima({ abierto, onCerrar, onCreada }) {
  ...
  const guardar = async () => {
    ...
    try {
      ...
      toast.success(creadas === 1 ? 'Salida registrada.' : `${creadas} salidas registradas.`)
      onCreada()
      cerrar()
    } catch (err) {
  ...
  return (
    <Modal abierto={abierto} titulo="Registrar salida — Nota de Entrega de Materia Prima" onCerrar={cerrar} maxWidth="max-w-4xl">
