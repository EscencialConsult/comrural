...
import DatosRecepcionLote from '../components/formularios/DatosRecepcionLote.jsx'
import ControlDocumentos from '../components/formularios/ControlDocumentos.jsx'
import DatosTransporte from '../components/formularios/DatosTransporte.jsx'
import DatosProductoYCantidad from '../components/formularios/DatosProductoYCantidad.jsx'
import ResumenRecepcion from '../components/formularios/ResumenRecepcion.jsx'
import PesajeFinal from '../components/formularios/PesajeFinal.jsx'
...
// de la maqueta (ver cerebro-facundo/galeria-de-componentes/formularios/
// formulario-comrural.md y docs/formulario-inspeccion-materia-prima.md,
// que documenta el primero). Mismo criterio: section-aware contra el papel
// real, reusando los átomos de src/components/formularios/ que ya no saben
// nada de Calidad en particular (CabeceraFormulario, SeccionFormulario,
// FirmasResponsables, CampoObservaciones, AvisoFaltante).
//
...
//
// Los pesos (sección "Unidades de medida") solo se pueden cargar junto con
// el cierre (`complete: true`), y solo cuando Calidad ya resolvió — ver
// docs/warehouse-receipts.md y el comentario de cabecera de
// warehouseReceiptsService.js.
...
export default function PanelIngresoMateriaPrima() {
  ...
  // Calidad (solo materia prima, `nature === 'PM'`).
  useEffect(() => {
    if (!puedeVer) return
    let cancelado = false
    ...
    <main className="flex w-full flex-col gap-6 p-6 md:p-10">
      ...
      <SeccionFormulario numero={1} titulo="Datos de recepción">
        <DatosRecepcionLote
          ...
          onCambiarLote={(op) => op?.id && op.id !== lotId && navigate(`/panel/calidad/lotes/${op.id}/ingreso`)}
